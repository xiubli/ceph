...
class TestFscrypt(XFSTestsDev):
    ...
    def test_fscrypt_encrypt(self):
        ...
        # accordingly.
        proc = self.mount_a.client_remote.run(args=['env DIFF_LENGTH=0 '
            'sudo ./check -g encrypt'], cwd=self.xfstests_repo_path, stdout=StringIO(),
    ...
    def test_fscrypt_dummy_encryption_with_quick_group(self):
        ...
        proc = self.mount_a.client_remote.run(args=['evn DIFF_LENGTH=0 '
            'sudo ./check -g quick -E ./ceph.exclude'], cwd=self.xfstests_repo_path,
