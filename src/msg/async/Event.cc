...
{
  {
    ...
    // Index rather than iterate: the old deque loop re-read front()
    // every pass and so tolerated a callback queueing another event,
    // and there is no reason for the vector version to be less robust.
    // (A callback that did that would in fact deadlock on external_lock
    // long before the vector reallocated, but do not rely on it.)
    for (size_t i = 0; i < external_events.size(); i++) {
      if (EventCallbackRef e = external_events[i]; e)
        e->do_request(0);
    }
...
int EventCenter::process_events(unsigned timeout_microseconds,  ceph::timespan *working_dur)
{
  ...
  // Drivers either resize() or push_back() into this, so it must start
  // empty.  clear() keeps the capacity from the previous iteration.
  fired_events.clear();
  ...
  if (external_num_events.load()) {
    external_lock.lock();
    // external_events_process was cleared at the end of the last round,
    // so external_events comes back empty but keeps its capacity.
    external_events_process.swap(external_events);
    external_num_events.store(0);
    external_lock.unlock();
    numevents += external_events_process.size();
    // Callbacks may queue new external events; those land in
    // external_events (under the lock) and are picked up next round.
