...
void MDLog::_submit_entry(LogEvent *le, MDSLogContextBase *c)
{
  ...
  if (max_event_size > period / 2 && factor > 1.0) {
    uint64_t _period = factor * std::max<uint64_t>(period, max_event_size);
  ...
  if (le->get_type() == EVENT_SUBTREEMAP ||
    ...
    dout(10) << "submit_entry also starting new segment: last = "
	     ...
	     << event_seq << ", max_event_size = " << max_event_size
	     << ", period = " << period << dendl;
    _start_new_segment();
...
void MDLog::_submit_thread()
{
  ...
  while (!mds->is_daemon_stopping()) {
	...
	ls->offset = write_pos;
      ...
      max_event_size = (total_events % 10000) ? bl.length() : max_event_size;
