...
void MDCache::rejoin_reconnect_subtrees()
{
  ...
  if (mds->get_nodeid() == root_rank) {
    rejoin_reconnect_inode_finish(CEPH_INO_ROOT, 0);
  } else {
    auto fin = new C_MDC_RejoinReconnectInodeFinish(this, CEPH_INO_ROOT);
    discover_base_ino(CEPH_INO_ROOT, fin, root_rank);
  }
}
				   ...
				   bool discover, bool want_xlocked, mds_rank_t *hint)
{
  ...
  for (unsigned i = 0; i < ancestors.size(); i++) {
	...
	dir = in->get_parent_dir();
      ...
      if (!dir)
	continue;
      _open_ino_fetch_dir(ino, m, i == 0, dir, "");
      return 1;
    }
...
void MDCache::do_open_ino(inodeno_t ino, open_ino_info_t& info, int err)
{
  ...
  if (info.check_peers) {
    ...
    open_ino(parent_ino, mds->get_metadata_pool(),
...
void MDCache::fragment_mark_and_complete(const MDRequestRef& mdr)
{
  ...
  for (const auto& dir : info.dirs) {
    if (!dir->is_frozen_dir()) {
      ceph_assert(dir->is_freezing_dir());
      dir->add_waiter(MDSCacheObject::WAIT_FROZEN, gather.new_sub());
    }
  }
  if (gather.has_subs()) {
