...
int Migrator::export_dir(CDir *dir, mds_rank_t dest, bool recursive)
{
  ...
  if (dir->get_version() == 0)
    dir->fetch(new C_MDSInternalNoop);
...
void Migrator::export_frozen(dirfrag_t df, uint64_t tid)
{
  ...
  // send.
  stat.set_state(EXPORT_PREPPING);
  ...
  ceph_assert(stat.export_client_set.empty());
  get_export_client_set(dir, stat.export_client_set);

  MDSGatherBuilder gather(g_ceph_context);
  mds->server->flush_client_sessions(stat.export_client_set, gather);
  if (gather.has_subs()) {
...
bool Migrator::maybe_export_fast(export_state_t& stat)
{
  ...
  mds->mdlog->submit_entry(new EExport(dir->dirfrag(), stat.bound_vec, stat.peer, 0),
...
void Migrator::export_go_synced(dirfrag_t df, uint64_t tid)
{
  ...
  stat.set_state(EXPORT_EXPORTING);
...
void Migrator::handle_export_ack(const cref_t<MExportDirAck> &m)
{
  ...
  it->second.set_state(EXPORT_LOGGING);
  ceph_assert(g_conf()->mds_kill_export_at != 9);

  // log completion.
  //  include export bounds, to ensure they're in the journal.
  EExport *le = new EExport(dir->dirfrag(), it->second.bound_vec, it->second.peer, it->second.tid);
  ...
  it->second.ls = mds->mdlog->get_current_segment().get();
  it->second.ls->uncommitted_exports.insert(dir->dirfrag());
...
void Migrator::handle_export_finish_ack(const cref_t<MExportDirFinishAck> &m)
{
  ...
  if (mds->is_resolve() && !peer_resolve_gathered) {
    ...
    mds->mdlog->submit_entry(new EExportCommitted(it->first, stat.tid),
...
void Migrator::import_reverse(CDir *dir, import_state_iterator it, bool notify)
{
  ...
  // log our failure
  mds->mdlog->submit_entry(new EImportFinish(dir->dirfrag(), stat.peer, stat.tid, false));
...
void Migrator::import_finish(CDir *dir, import_state_t& stat, bool last)
{
  ...
  if (mds->is_resolve()) {
    ...
    mds->mdlog->submit_entry(new EImportFinish(dir->dirfrag(), stat.peer, stat.tid, true),
...
void Migrator::dump_export_states(Formatter *f)
{
  ...
  for (const auto& [df, state] : export_state) {
...
void Migrator::replay_export_dir(dirfrag_t base, const vector<dirfrag_t>& bound_vec,
				 mds_rank_t target, uint64_t tid, LogSegmentRef ls)
{
				   ...
				   const ref_t<MMDSResolveAck>& ack)
{
  ...
  if (!mds->is_resolve()) {
	...
	continue;
      ceph_assert(stat.state == EXPORT_FINISHED);
      mds->mdlog->submit_entry(new EExportCommitted(df, stat.tid),
