...
public:
  ...
  void replay_export_dir(dirfrag_t base, const std::vector<dirfrag_t>& bound_vec,
			 mds_rank_t target, uint64_t tid, LogSegmentRef ls);
  void replay_export_committed(dirfrag_t base, uint64_t tid);

  void replay_import_start(dirfrag_t base, const std::vector<dirfrag_t>& bound_vec,
			   mds_rank_t from, uint64_t tid);
  void replay_import_finish(dirfrag_t base, uint64_t tid, bool success);

  void add_ambiguous_imports(std::map<mds_rank_t, ref_t<MMDSResolve>>& resolves,
			     const std::set<mds_rank_t>& resolve_set);
...
protected:
  ...
  typedef std::map<dirfrag_t, import_state_t>::iterator import_state_iterator;
