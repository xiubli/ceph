...
class CommonSafeTimerThread : public Thread {
  CommonSafeTimer<L> *parent;
public:
  explicit CommonSafeTimerThread(CommonSafeTimer<L> *s) : parent(s) {}
  void *entry() override {
    parent->timer_thread();
    return NULL;
  }
...
template <class Lock>
CommonSafeTimer<Lock>::CommonSafeTimer(CephContext *cct_, Lock &l, bool safe_callbacks)
  : cct(cct_), lock(l),
...
template <class Lock>
CommonSafeTimer<Lock>::~CommonSafeTimer()
{
  ceph_assert(thread == NULL);
}

template <class Lock>
void CommonSafeTimer<Lock>::init()
{
  ldout(cct,10) << "init" << dendl;
  thread = new CommonSafeTimerThread<Lock>(this);
  thread->create("safe_timer");
}

template <class Lock>
void CommonSafeTimer<Lock>::shutdown()
{
...
template <class Lock>
void CommonSafeTimer<Lock>::timer_thread()
{
...
template <class Lock>
Context* CommonSafeTimer<Lock>::add_event_after(double seconds, Context *callback)
{
  return add_event_after(ceph::make_timespan(seconds), callback);
}

template <class Lock>
Context* CommonSafeTimer<Lock>::add_event_after(ceph::timespan duration, Context *callback)
{
  ceph_assert(ceph_mutex_is_locked(lock));
...
template <class Lock>
Context* CommonSafeTimer<Lock>::add_event_at(CommonSafeTimer<Lock>::clock_t::time_point when, Context *callback)
{
  ceph_assert(ceph_mutex_is_locked(lock));
...
template <class Lock>
Context* CommonSafeTimer<Lock>::add_event_at(ceph::real_clock::time_point when, Context *callback)
{
  ceph_assert(ceph_mutex_is_locked(lock));
...
template <class Lock>
bool CommonSafeTimer<Lock>::cancel_event(Context *callback)
{
  ceph_assert(ceph_mutex_is_locked(lock));
...
template <class Lock>
void CommonSafeTimer<Lock>::cancel_all_events()
{
...
template <class Lock>
void CommonSafeTimer<Lock>::dump(const char *caller) const
{
  if (!caller)
    caller = "";
