...
template <class L> class CommonSafeTimerThread;
...
class CommonSafeTimer
{
  CephContext *cct;
  Lock& lock;
  ceph::ceph_condition_variable<Lock> cond;
  bool safe_callbacks;

  friend class CommonSafeTimerThread<Lock>;
  class CommonSafeTimerThread<Lock> *thread;
...
public:
   ...
   * */
  CommonSafeTimer(CephContext *cct, Lock &l, bool safe_callbacks=true);
